...
execute as @e[type=area_effect_cloud,tag=combat.aec] run data modify entity @s Owner set from entity @a[tag=combat.attacker,limit=1] UUID
tag @e[type=area_effect_cloud,tag=combat.aec] remove combat.aec
##========================================================
tag @a[tag=combat.attacker] remove combat.attacker
