...
execute unless score $out utils.math matches 0 as @e[type=!player,tag=combat.registered] if score @s combat.id = $hit combat.id run function combat:zprivate/hitentity/damage